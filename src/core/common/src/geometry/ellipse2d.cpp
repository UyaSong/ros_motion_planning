...
{
  return include_bound ? rmp::common::math::less(distTo(p), 1.0) || rmp::common::math::equal(distTo(p), 1.0) :
                         rmp::common::math::less(distTo(p), 1.0);
}
...
Eigen::Vector2d Ellipse2d::closestPoint(const std::vector<Vec2d>& p_list) const
{
  ...
  {
    double d = distTo(p);
    if (rmp::common::math::large(min_dist, d))
    {
      min_dist = d;
      closest_point[0] = p.x();
      closest_point[1] = p.y();
...
double Ellipse2d::distTo(const Vec2d& p) const
{
  ...
  auto related_vec = C_.inverse() * (temp - d_);
