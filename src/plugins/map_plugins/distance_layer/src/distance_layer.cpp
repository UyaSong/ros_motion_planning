...
double* DistanceLayer::getEDFPtr() const
{
  return edf_ptr_;
}

int DistanceLayer::getSizeInCellsX() const
{
  return static_cast<int>(edf_[0].size());
}

int DistanceLayer::getSizeInCellsY() const
{
  return static_cast<int>(edf_.size());
}
...
void DistanceLayer::updateCosts(costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j)
{
  ...
  int size_y = static_cast<int>(master_grid.getSizeInCellsY());
  edf_manager_->setGridMap(master_grid.getCharMap(), size_x, size_y);
  ...
  edf_ptr_ = new double[size_x * size_y];
  for (int y = 0; y < size_y; y++)
  {
    for (int x = 0; x < size_x; x++)
    {
      edf_ptr_[x + size_x * y] = edf_[y][x];
    }
  }
